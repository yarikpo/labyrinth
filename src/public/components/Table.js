    ...
    importLevel1() {
        for (let i = 0; i < map1.map.length; ++i) {
            for (let j = 0; j < map1.map[0].length; ++j) {
                ...
                if (map1.map[i][j] == 'A') this.field[i][j] = new DoorAField({xCord: j, yCord: i, ctx: this.ctx});
                if (map1.map[i][j] == 'a') this.field[i][j] = new KeyAField({xCord: j, yCord: i, ctx: this.ctx});
                if (map1.map[i][j] == 'B') this.field[i][j] = new DoorBField({xCord: j, yCord: i, ctx: this.ctx});
                if (map1.map[i][j] == 'b') this.field[i][j] = new KeyBField({xCord: j, yCord: i, ctx: this.ctx});
                if (map1.map[i][j] == 'C') this.field[i][j] = new DoorCField({xCord: j, yCord: i, ctx: this.ctx});
                if (map1.map[i][j] == 'c') this.field[i][j] = new KeyCField({xCord: j, yCord: i, ctx: this.ctx});
            }
        }
    }
}
